...
from solver.ceoh.problems.descriptions.puzzle.puzzle_astar_edu_uninformed_astar.prompts import GetPrompts
...
# ---------------------------------------------------------------------
class PUZZLE_ASTAR:
    def __init__(self, instance_file=None, code_string=None, paras=None):
        """
        paras may override:
         - TIMEOUT_SECONDS
         - NUM_EVAL_INSTANCES
...
def score_state(state):
    ...
    # Row/column linear-order penalties (soft constraints)
    for row in range(n):
        last = -1
        for col in range(n):
            v = state[row][col]
            if v != 0:
                if last != -1 and (v - 1) % n < (last - 1) % n:
                    score += 2 * abs((v - 1) % n - (last - 1) % n)
                last = v
...
"""
    ...
    paras_instance.set_paras(
        ...
        # MAX_MOVES=200,
        # N_WORKERS=10,
        # MAX_EVALUATED_NODES=1_000_000
    )

    puzzle_solver = PUZZLE_ASTAR(code_string=code_string, paras=paras_instance)
